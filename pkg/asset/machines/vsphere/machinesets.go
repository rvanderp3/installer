...
func getVCenterFromServerName(server string, platformSpec *vsphere.Platform) (*vsphere.VCenterSpec, error) {
	for _, vCenter := range platformSpec.VCenters {
		if vCenter.Server == server {
			return &vCenter, nil
		}
	}
	return nil, errors.Errorf("%s is not a defined vCenter server", server)
}

func getFailureDomain(domainName string, platformSpec *vsphere.Platform) (*vsphere.FailureDomainSpec, error) {
...
func getDefinedZones(platformSpec *vsphere.Platform, controlPlane bool) (map[string]*vsphere.Platform, error) {
	...
	for _, deploymentZone := range platformSpec.DeploymentZones {
		...
		vCenter, err := getVCenterFromServerName(deploymentZone.Server, platformSpec)
		if err != nil {
			return nil, err
		}
...
func MachineSets(clusterID string, config *types.InstallConfig, pool *types.MachinePool, osImage, role, userDataSecret string) ([]*machineapi.MachineSet, error) {
	...
	if numOfAZs > 0 {
		...
		for idx := range azs {
			...
			machinesetName := fmt.Sprintf("%s-%s-%s", clusterID, pool.Name, desiredZone)
